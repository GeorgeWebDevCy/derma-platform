...
export default async function AdminDashboard() {
    ...
    const [usersCount, doctorsCount, patientsCount, consultationsCount, pendingCount, assignedCount, completedCount, recent] =
        await Promise.all([
            prisma.user.count(),
    ...
    const totalForBar = pendingCount + assignedCount + completedCount || 1
                        ...
                        {recent.map((c) => (
                            <div key={c.id} className="rounded-md border border-white/5 bg-white/5 p-3">
                                ...
                                <p className="text-xs text-white/60">
                                    {t.admin.patient}: {c.patient?.email ?? "—"}
                                </p>
                                <p className="text-xs text-white/60">
...
function Bar({ label, value, width, tone }: { label: string; value: number; width: string; tone: "amber" | "cyan" | "emerald" }) {
